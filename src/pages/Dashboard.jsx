...
export default function Dashboard() {
  ...
  async function load() {
    const { data, error } = await supabase.from('participants').select('checked_in, paid, race_number')
    ...
    setStats({
      ...
      awaitingBib: data.filter(p => p.race_number == null).length,
      finished,
    })
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      <div className="page-sub">{RACE_NAME} — race day overview</div>
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card">
          <div className="stat-number" style={{ color: stats.awaitingBib > 0 ? 'var(--accent)' : 'var(--muted)' }}>
            {stats.awaitingBib}
          </div>
          <div className="stat-label">Awaiting Bib #</div>
        </div>
      </div>
      ...
      <div className="stat-grid" style={{ marginBottom: 28 }}>
        <div className="stat-card">
          ...
          <div className="text-muted text-sm" style={{ marginTop: 2 }}>{stats.total} registered</div>
        </div>
        <div className="stat-card">
